...
def compute_canslim_scores(ticker: str, data: Dict[str, pd.DataFrame], price_df: pd.DataFrame) -> CanslimScores:
    ...
    if isinstance(a, pd.DataFrame) and len(a) >= 4:
        ...
        try:
            ...
            pass
    # N - New products/services/management and 52‑week high
    # Use proximity to 52‑week high as a proxy: near high => strong innovation sentiment
    if not price_df.empty:
        try:
            high_52w = price_df["High"].rolling(window=252).max().iloc[-1]
            current_price = price_df["Close"].iloc[-1]
            near_ratio = current_price / high_52w if high_52w > 0 else np.nan
            # Score emphasises closeness to high: 90% of high => 100 pts
            score = min(max((near_ratio / 0.90) * 100.0, 0), 100)
            scores.new_products = round(score, 2)
        except Exception:
            pass
    ...
    try:
        ...
        pass
    # M - Market direction: evaluate S&P 500 (or broader market) 3‑month trend
    try:
        sp = yf.download("^GSPC", period="6mo", interval="1d", progress=False)
        if not sp.empty and len(sp) >= 63:
            ret_mkt = sp["Close"].iloc[-1] / sp["Close"].iloc[-63] - 1.0
            # Market trending up strongly (>10%) => 100 pts; trending down (<0) => 0 pts
            score = min(max((ret_mkt / 0.10) * 100.0, 0), 100)
            scores.market_direction = round(score, 2)
    except Exception:
        pass
    return scores
...
def update_google_sheet(sheet_name: str, df: pd.DataFrame) -> None:
    ...
    if not GSPREAD_AVAILABLE:
        st.error("gspread がインストールされていません。Google Sheets への書き込みは利用できません。")
        return
    creds_path = os.getenv("GOOGLE_CREDENTIALS_JSON")
    spreadsheet_id = os.getenv("GOOGLE_SPREADSHEET_ID")
    if not creds_path or not spreadsheet_id:
        st.error("Google Sheets の認証情報が設定されていません。環境変数を確認してください。")
        return
    try:
        ...
        st.error(f"Google Sheets への書き込み中にエラーが発生しました: {e}")
...
def post_to_notion(page_title: str, report_md: str) -> None:
    ...
    if not NOTION_AVAILABLE:
        st.warning("notion_client がインストールされていないため、Notion 連携は実行されません。")
        return
    notion_token = os.getenv("NOTION_TOKEN")
    notion_db_id = os.getenv("NOTION_DATABASE_ID")
    if not notion_token or not notion_db_id:
        st.error("Notion の認証情報が設定されていません。環境変数を確認してください。")
        return
    try:
        ...
        st.error(f"Notion への投稿中にエラーが発生しました: {e}")
